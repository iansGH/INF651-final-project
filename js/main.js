...
// 1. createElemWithText DONE
// a. Receives up to 3 parameters DONE
// b. 1st parameter is the HTML element string name to be created (h1, p, button, etc) DONE
// c. Set a default value for the 1st parameter to “p” DONE
// d. 2nd parameter is the textContent of the element to be created DONE
// e. Default value of the 2nd parameter is “” DONE
// f. 3rd parameter is a className if one is to be applied (optional) DONE?
// g. Use document.createElement() to create the requested HTML element DONE
// h. Set the other desired element attributes. DONE
// i. Return the created element. DONE
...
// c. Receives users JSON data as a parameter DONE
// d. Returns undefined if no parameter received DONE
// e. Loops through the users data DONE
...
// 3. toggleCommentSection DONE
// a. Receives a postId as the parameter DONE
// b. Selects the section element with the data-post-id attribute equal to the postId
// received as a parameter DONE
...
// desired. DONE
// e. Toggles the class 'hide' on the section element DONE
// f. Return the section element DONE
function toggleCommentSection(postId) {
    if(!postId) return; 
    const commentSection = document.querySelector(`section[data-post-id="${postId}"`);
    if(commentSection == null) return null;
    commentSection.classList.toggle("hide"); 
    return commentSection;
}

// 4. toggleCommentButton DONE
// a. Receives a postId as the parameter DONE
...
// Comments' DONE
// e. Suggestion (not required) for above: try a ternary statement DONE
// f. Return the button element DONE
function toggleCommentButton(postId){
    if(!postId) return; 
    const commentButton = document.querySelector(`button[data-post-id="${postId}"`);
    if(commentButton == null) return null;
...
// a. Receives a parentElement as a parameter DONE
// b. Define a child variable as parentElement.lastElementChild DONE
// c. While the child exists...(use a while loop) DONE
// d. Use parentElement.removeChild to remove the child in the loop DONE
// e. Reassign child to parentElement.lastElementChild in the loop DONE
...
// event and postId as parameters DONE
...
function addButtonListeners(){
    ...
    for(let i = 0; i < mainButtons.length; i++){
        const postId = mainButtons[i].dataset.postId;
        mainButtons[i].addEventListener("click", function(event){ toggleComments(event, postId)}, false);
    }
...
function removeButtonListeners(){
    ...
    /*for(let i = 0; i < mainButtons.length; i++){
        const postId = mainButtons[i].dataset.postId;
        mainButtons[i].removeEventListener("click", function(event){ toggleComments(event, postId)}, false);
    }*/
...
function createComments(jsonComments){
    ...
    jsonComments.forEach((comment) => {
        ...
        h3Element = createElemWithText('h3', comment.name);
        p1Element = createElemWithText('p', comment.body);
        p2Element = createElemWithText('p', `From: ${comment.email}`)
...
function populateSelectMenu(jsonData){
    ...
    optionsElements.forEach((element) => {
        selectMenuElement.append(element);
    })
    return selectMenuElement;
}


// NOTE: The next functions use Async / Await to request data from an API. We cover this 
//in Week 13. I do not recommend proceeding beyond this point until you have completed 
//the learning module for Week 13.
...
// Therefore, these functions will also need to be async. When they call the API functions, 
//they will need to await data from those functions.
...
// l. Create another p element with text of 
...
// the article element. //section element is after this instruction
// q. Create a variable named section equal to the result of 
//await displayComments(post.id); 
// r. Append the section element to the article element
// s. After the loop completes, append the article element to the fragment //before?
...
async function createPosts(jsonData){
    ...
    //jsonData.forEach(async (post) => { //unable to use forEach but is it possible?
    for(let i = 0; i < jsonData.length; i++){
        ...
        p1 = createElemWithText('p', post.body);
        p2 = createElemWithText('p', `Post ID: ${post.id}`);

        const author = await getUser(post.userId);
        p3 = createElemWithText('p', `Author: ${author.name} with ${author.company.name}`);
        p4 = createElemWithText('p', `${author.company.catchPhrase}`);

        button = createElemWithText('button', 'Show Comments');
        ...
        fragment.append(articleElement); //before the loop comples instead of after? 
    }//) //parenthesis paired with forEach
...
//NOTE: This is the last group of functions. I call them “procedural functions” because 
//they exist to pull the other functions together in an order that allows the web app 
//to function as it should.This means their sole purpose is to call dependencies with 
//the correct data in the proper order.

// 17. toggleComments
// a. Dependencies: toggleCommentSection, toggleCommentButton CHECKED
// b. Receives 2 parameters: (see addButtonListeners function description) CHECKED
    // i. The event from the click event listener is the 1st param DONE
    // ii. Receives a postId as the 2nd parameter DONE
// c. Sets event.target.listener = true (I need this for testing to be accurate) DONE
// d. Passes the postId parameter to toggleCommentSection() DONE
// e. toggleCommentSection result is a section element CHECK
// f. Passes the postId parameter to toggleCommentButton() DONE 
// g. toggleCommentButton result is a button CHECKED
// h. Return an array containing the section element returned from
// toggleCommentSection and the button element returned from
// toggleCommentButton: [section, button] DONE 
...
async function selectMenuChangeEventHandler(event){
    ...
    document.getElementById("selectMenu").setAttribute("disabled", "true");
...
function initApp(){
    initPage();
    //document.getElementById("selectMenu").addEventListener("change", function (e) { selectMenuChangeEventHandler(e)}, false);
    //Notice how much syntax matters as demonstrated by #
    document.querySelector("#selectMenu").addEventListener("change", function (e) { selectMenuChangeEventHandler(e)}, false); 
}

// NOTE: There is one last step to get your app to function correctly. I cannot test for this, 
// but you must apply it to call the script into action.
